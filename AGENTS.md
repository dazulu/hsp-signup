...
- `MobileAppDataContext` (`src/context/mobile-app-data.tsx`) — global context providing `{ data, events, stravaData, loading, refresh, refreshContentful }`. Owns all remote data: Contentful `MOBILE_APP_DATA` entry, Contentful events, and Strava. `refresh(force?)` fetches all three and returns `Promise<void>` (used by pull-to-refresh). `refreshContentful()` fetches Contentful only and returns `Promise<void>` (used by `useFocusEffect` on screens). On web, `refresh()` skips events and Strava. `data` uses a 1-hour TTL cache (`app_contentful_mobile_app_data`); events use a 1-hour TTL cache; Strava uses a 30-minute TTL cache; `force=true` bypasses TTL but always writes fresh data back. On startup, cached values for all three are pre-populated into state before `refresh()` fires so components render immediately.
